...
{
	public interface IKT_TW_SPLRepository
	{
		Task<List<KT_TW_SPL>> GetListAsync(FilterModel filterModel);
		Task<List<KT_TW_SPL>> GetExistNums(List<string> existNUMs);

		Task<KT_TW_SPL> GetByIdAsync(long id);
		Task CreateAsync(KT_TW_SPL entity);
		Task UpdateAsync(KT_TW_SPL entity);
		Task UpdateRangeAsync(IEnumerable<KT_TW_SPL> entities);
		Task CreateRangeAsync(IEnumerable<KT_TW_SPL> entities);
		Task DeleteAsync(long id);
		Task<List<ResultModel>> GetTotal(FilterModel filterModel);
		Task<PaginatedList<KT_TW_SPL>> GetAllAsync(FilterModel filterModel);
		Task<List<KT_TW_SPL>> GetListByNumAsync(string num);
	}

	public class KT_TW_SPLRepository(NFCDbContext context) : IKT_TW_SPLRepository
	{
		private readonly NFCDbContext _context = context;

		public async Task<List<KT_TW_SPL>> GetExistNums(List<string> existNUMs)
		{
			...
			try
			{
				...
				var result = _context.KT_TW_SPLs
					.FromSqlRaw($"SELECT * FROM KT_TW_SPLs WHERE NUM IN ({existNUMsSetString})", parameters.ToArray())
					.ToList();
		...
		public async Task<List<KT_TW_SPL>> GetListAsync(FilterModel filterModel)
		{
			var query = _context.KT_TW_SPLs as IQueryable<KT_TW_SPL>;
		...
		public async Task<List<KT_TW_SPL>> GetListByNumAsync(string num)
		{
			return await _context.KT_TW_SPLs.Where(x => x.NUM.Contains(num)).Include(x => x.ProductionLine).OrderByDescending(x => x.DateTime).ToListAsync();
		}
		...
		public async Task<PaginatedList<KT_TW_SPL>> GetAllAsync(FilterModel filterModel)
		{
			// Tạo chuỗi truy vấn SQL
			var sqlQuery = @"
							WITH RankedKT_TW_SPLs AS
							(
								SELECT h.*, 
			...
			var items = await _context.KT_TW_SPLs.FromSqlRaw(sqlQuery, parameters).AsNoTracking().ToListAsync();
			...
			return new PaginatedList<KT_TW_SPL>(items, count, filterModel.PageNumber, filterModel.PageSize);

		}
		public async Task<KT_TW_SPL> GetByIdAsync(long id)
		{
			return await _context.KT_TW_SPLs.Include(x => x.ProductionLine).FirstOrDefaultAsync(x => x.Id == id);
		}

		public async Task CreateAsync(KT_TW_SPL entity)
		{
			_context.KT_TW_SPLs.Add(entity);
			await _context.SaveChangesAsync();
		}

		public async Task UpdateAsync(KT_TW_SPL entity)
		{
			_context.KT_TW_SPLs.Update(entity);
			await _context.SaveChangesAsync();
		}
		public async Task UpdateRangeAsync(IEnumerable<KT_TW_SPL> entities)
		{
			bool success = false;
			int retryCount = 0;
			int maxRetries = 3;
		...
		public async Task CreateRangeAsync(IEnumerable<KT_TW_SPL> entities)
		{
			bool success = false;
			int retryCount = 0;
			int maxRetries = 3;
		...
		public async Task DeleteAsync(long id)
		{
			...
			if (entity != null)
			{
				_context.KT_TW_SPLs.Remove(entity);
				await _context.SaveChangesAsync();
			}
		}
	}
